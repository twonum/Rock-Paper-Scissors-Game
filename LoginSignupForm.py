...
connection_string = 'DRIVER={SQL Server};SERVER=XPS-15;DATABASE=RockPaper;Trusted_Connection=yes;'
...
def youPick(yourChoice):
    ...
    if counter == turns:
        ...
        if userWin > compWin:
            ...
            message = 'You Tied!!'
        insertToLeaderBoard(USER, userWin, compWin)
        top = Toplevel()
        top.title("Result")
        top.geometry('300x300')
        confetiImg = PhotoImage(file=r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\confeti.gif")
...
def start():
    ...
    # ----------------Image set----------------
    rHandPhoto = PhotoImage(file=r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\rHand.png")
    pHandPhoto = PhotoImage(file=r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\pHand.png")
    sHandPhoto = PhotoImage(file=r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\sHand.png")
    rock = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\Rockimg.jpg")
    rockImage = ImageTk.PhotoImage(rock)
    paper = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\Paperimg.jpg")
    paperImage = ImageTk.PhotoImage(paper)
    scissors = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\Scissorsimg.jpg")
    scissorImage = ImageTk.PhotoImage(scissors)
    win = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\YouWin.jpg")
    winImage = ImageTk.PhotoImage(win)
    lose = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\YouLose.jpg")
    loseImage = ImageTk.PhotoImage(lose)
    tie = Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\YouTie.jpg")
...
def welcomeUserPage():
    ...
    if UserGender == "Male":
        maleimg = ImageTk.PhotoImage(Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\Male.jpg"))
        maleLabel = Label(root ,image=maleimg)
        maleLabel.image = maleimg
        maleLabel.grid(row=1, column=0)
        compimg = PhotoImage(file = r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\computer.png")
        compLabel = Label(root, image=compimg)
        compLabel.image = compimg
        compLabel.grid(row=1, column=1)
    else:
        femaleimg = ImageTk.PhotoImage(Image.open(r"C:\Users\DELL\OneDrive\Desktop\Self Study\Personal Projects\RockPaper\Rock-Paper-Scissors-Game-main\Images\Female.jpg"))
...
def check():
    ...
    if username.get() == "" or password.get() == "" or data is None:
...
# Function to create LeaderBoard Table if it doesn't already exist
def LeaderboardCreate():
    conn = pyodbc.connect(connection_string)
    c = conn.cursor()
    # Check if the LEADERBOARD table already exists
    c.execute("""
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'LEADERBOARD')
    BEGIN
...
def insertToLeaderBoard(USER, userWin, compWin):
    conn = pyodbc.connect(connection_string)
    c = conn.cursor()
    c.execute("SELECT * FROM LEADERBOARD WHERE firstname = ?", (USER,))
    data = c.fetchone()
    if data is None:
        # If no record is found, insert a new row
        c.execute("INSERT INTO LEADERBOARD (firstname, GamesWon, GamesLost, GamesTied, GamesPlayed, WiningRate) VALUES (?, 0, 0, 0, 0, 0)", (USER,))
        conn.commit()
        c.execute("SELECT * FROM LEADERBOARD WHERE firstname = ?", (USER,))
        data = c.fetchone()
    ...
    c.execute("""
        ...
        WiningRate = ?        
        WHERE firstname = ?
        """, (won, lost, tied, played, rate, USER))
    conn.commit()
    conn.close()
...
def getLeaderboard():
    ...
    boardb.configure(bg='white')
    board = tk.Frame(boardb, bg='white', padx=10, pady=10)
    ...
    data = c.fetchall()
    rowno = 2
    Heading = tk.Label(board, text="Leaderboard", bg='brown', fg='white', width=45, pady=10, padx=10)
    Heading.config(font=("Times", 20, "bold"))
    Heading.grid(row=0, column=0, columnspan=6, padx=5, pady=5)

    name = tk.Label(board, text="Name", width=15, bg='#0069b3', fg='white', padx=5, pady=5)
    name.grid(row=1, column=0, padx=2, pady=2)
    won = tk.Label(board, text="Games Won", width=15, bg='#a617ff', fg='white', padx=5, pady=5)
    won.grid(row=1, column=1, padx=2, pady=2)
    lost = tk.Label(board, text="Games Lost", width=15, bg='#e01717', fg='white', padx=5, pady=5)
    lost.grid(row=1, column=2, padx=2, pady=2)
    tied = tk.Label(board, text="Games Tied", width=15, bg='#e77c00', fg='white', padx=5, pady=5)
    tied.grid(row=1, column=3, padx=2, pady=2)
    played = tk.Label(board, text="Games Played", width=15, bg='#30b000', fg='white', padx=5, pady=5)
    played.grid(row=1, column=4, padx=2, pady=2)
    rate = tk.Label(board, text="Winning Rate", width=15, bg='#ff1f60', fg='white', padx=5, pady=5)
    rate.grid(row=1, column=5, padx=2, pady=2)

    for record in data:
        name = tk.Label(board, text=record[0], width=15, bg='#3d7eac', fg='white', padx=5, pady=5)
        name.grid(row=rowno, column=0, padx=2, pady=3)
        won = tk.Label(board, text=record[1], width=15, bg='#b846ff', fg='white', padx=5, pady=5)
        won.grid(row=rowno, column=1, padx=2, pady=3)
        lost = tk.Label(board, text=record[2], width=15, bg='#e33f3f', fg='white', padx=5, pady=5)
        lost.grid(row=rowno, column=2, padx=2, pady=3)
        tied = tk.Label(board, text=record[3], width=15, bg='#e38d2a', fg='white', padx=5, pady=5)
        tied.grid(row=rowno, column=3, padx=2, pady=3)
        played = tk.Label(board, text=record[4], width=15, bg='#62be40', fg='white', padx=5, pady=5)
        played.grid(row=rowno, column=4, padx=2, pady=3)
        rate = tk.Label(board, text=record[5], width=15, bg='#ff5385', fg='white', padx=5, pady=5)
    ...
    light = tk.Button(boardb, text="Light Theme", pady=7, command=changeLight, bg='black', fg='white', width=30)
    light.grid(row=1, column=0, pady=(0, 20))
    dark = tk.Button(boardb, text="Dark Theme", pady=7, command=changeDark, bg='black', fg='white', width=30)
    dark.grid(row=1, column=1, pady=(0, 20))
    exit = tk.Button(boardb, text="Exit", pady=7, command=exitall, bg='red', fg='white', width=30)
...
def insert():
    ...
    c1 = conn1.cursor()
    c1.execute("""
        INSERT INTO USERS (firstname, lastname, username, password, gender)
        VALUES (?, ?, ?, ?, ?)
    """, (fname.get(), lname.get(), username.get(), password.get(), gender.get()))
    conn1.commit()

    conn2 = pyodbc.connect(connection_string)
    c2 = conn2.cursor()
    c2.execute("""
        INSERT INTO LEADERBOARD (firstname, GamesWon, GamesLost, GamesTied, GamesPlayed, WiningRate)
        VALUES (?, 0, 0, 0, 0, 0)
    """, (fname.get(),))
    conn2.commit()
